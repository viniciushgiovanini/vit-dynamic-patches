...
class CustomPatchEmbedding(nn.Module):
    def __init__(
        ...
        elif argumentos.pde == "espiral_grid":
            print("Abordagem selecionada: Seleção por Espiral")
            self.centers_zigzag_espiral = self.load_dict(
    ...
    def forward(self, x, **kwargs):
        ...
        for b in range(batch_size):
            ...
            ############################################################
            if (
                self.abordagem_selecionada != "grid"
                and self.abordagem_selecionada != "sr"
                and self.abordagem_selecionada != "zigzag"
                and self.abordagem_selecionada != "espiral"
                and self.abordagem_selecionada != "espiral_grid"
            ):
                try:
            ...
            ############################################################
            if (
                self.abordagem_selecionada == "espiral"
                or self.abordagem_selecionada == "zigzag"
                or self.abordagem_selecionada == "espiral_grid"
            ):
                try:
...
class ViTCustomEmbeddings(nn.Module):
    def __init__(
        self,
        config: ViTConfig,
        custom_patch_embedding: nn.Module,
        ...
        self.patch_embeddings = custom_patch_embedding
        self.position_embeddings = nn.Parameter(pretrained_pos_embed.clone())
...
class ModeloCustomConv2dEmbeddings(pl.LightningModule):
    def __init__(
        self,
        num_class,
        learning_rate,
        ...
        super(ModeloCustomConv2dEmbeddings, self).__init__()
        ...
        custom_patch_embedding = CustomPatchEmbedding(
        ...
        pretrained_pos_embed = base_model.embeddings.position_embeddings.data
        self.model.vit.embeddings = ViTCustomEmbeddings(
            config=base_model.config,
            custom_patch_embedding=custom_patch_embedding,
            pretrained_pos_embed=pretrained_pos_embed,
        )

        centers = self.load_dict(
            "./data/centros_pre_salvos/espiral_centers.pkl"
        )

        self.model.vit = ViTCustomEmbeddings.reoorder_position_embbeddings(
            self.model.vit, centers
        )
