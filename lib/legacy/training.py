...
from lib.legacy.modelo import Modelo
from lib.legacy.modelo_binario import ModeloBin
from lib.legacy.modelo_custom import ModeloCustom
from lib.legacy.modelo_custom_conv2d import ModeloCustomConv2d
...
model = Modelo(num_classes, learning_rate, args)

###########################
# Cria Logger para Metricas
###########################
csv_logger = CSVLogger(save_dir="./lightning_logs/", name="csv_file")
...
trainer = pl.Trainer(
    ...
    limit_val_batches=total_steps,
    log_every_n_steps=1,
    logger=[csv_logger, TensorBoardLogger("./lightning_logs/")],
    accelerator="gpu",
    devices="auto",
    callbacks=[checkpoint_callback, ModelSummary(max_depth=10)],
)
...
plt.subplot(1, 2, 1)
plt.plot(
    resultados["epoch"], resultados["train_accuracy"], label="Train Accuracy"
)
plt.plot(
    resultados["epoch"], resultados["val_accuracy"], label="Validation Accuracy"
)
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
...
print(f"Acurácia no conjunto de validacao: {accuracy * 100:.2f}%")
...
accuracy = calcular_acuracia_multiclasse(model, test_loader)
print(
    f"Acurácia no conjunto de validacao (Melhor ponto do modelo): {accuracy * 100:.2f}%"
)
